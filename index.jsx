...
const ATMDeposit = ({ onChange, isDeposit }) => {
  ...
  console.log(`isDeposit : ${Number(isDeposit)}`)
  return (
    <label className="label huge">
      <h2 className="header" id="id-header">{choice[Number(!isDeposit)]}</h2>
...
const App = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (deposit > accountState && !isDeposit) {
      alert('NOT ENOUGH BALANCE')
    } else {
      let newBalance = isDeposit ? accountState + deposit : accountState - deposit;
